    ...
    async run(client, message, args) {
        try{
             ...
             await   message.guild.roles.filter(r => r.name !== message.guild.member(client.user.id).highestRole.name).forEach(r => {
                    if (r.comparePositionTo(message.guild.member(client.user.id).highestRole) > 0){
                        let havnthighest = new RichEmbed()
                            ...
                            .setDescription(`Botun Rolü Sunucudaki En Yüksek Rol Değildir, Bu Yedeği Yüklerken Bazı Hatalara Neden Olabilir. !
            ...
            save();
                ...
                .setTitle(`${info}  Info`)
                .setDescription(`Sunucu yedeğiniz başarılı bir şekilde oluşturuldu **${message.guild.name}** yedeğiniz'in idisi \`${id}\``)
